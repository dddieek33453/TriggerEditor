...
namespace Triggers {
    public class TriggerWindow : EditorWindow {
        ...
        Dictionary<TriggerAction, TriggerEditorViewData> triggerEditorViewDataDic = new Dictionary<TriggerAction, TriggerEditorViewData>();
        ...
        //}
        public class TriggerEditorViewData {
            public TriggerActionEditor triggerEditor;
            public SerializedObject serializedEditor;
        }

        // 잘 안됨
        void TriggerEnable(TriggerAction trigger) {
            SerializedObject triggerSO;
            TriggerActionEditor triggerEditor;
            if (!triggerEditorViewDataDic.ContainsKey(trigger)) {
                triggerEditor = (TriggerActionEditor)Editor.CreateEditor(trigger);
                triggerSO = new SerializedObject(trigger);
                TriggerEditorViewData viewData = new TriggerEditorViewData();
                viewData.serializedEditor = triggerSO;
                viewData.triggerEditor = triggerEditor;
                triggerEditorViewDataDic.Add(trigger, viewData);
            }
            else {
                triggerSO = triggerEditorViewDataDic[trigger].serializedEditor;
                triggerEditor = triggerEditorViewDataDic[trigger].triggerEditor;
            }
        ...
        void ActionReorderableSetting() {
            ...
            ActionReorderable.drawElementCallback = (rect, index, isActive, isFocused) => {
                ...
                rect.height = 18;
                if (!triggerEditorViewDataDic.ContainsKey(trigger)) {
                    TriggerEnable(trigger);
                }
                TriggerActionEditor triggerEditor = triggerEditorViewDataDic[trigger].triggerEditor;
                SerializedObject triggerSO = triggerEditorViewDataDic[trigger].serializedEditor;
                triggerEditor.OnTriggerEnable(triggerSO);
            ...
            if (selectActionList.Count > 1) {
                ...
                if (ActionReorderable.index == -1) {
                    if (m_TriggerController.Sequence.Count > 0) {
                        TriggerAction triggerAction = m_TriggerController.Sequence[m_TriggerController.Sequence.Count - 1];
                        triggerEditorViewDataDic.Remove(triggerAction);
                        m_TriggerController.Sequence.RemoveAt(m_TriggerController.Sequence.Count - 1);
                    }
                }
                else {
                    TriggerAction triggerAction = m_TriggerController.Sequence[ActionReorderable.index];
                    triggerEditorViewDataDic.Remove(triggerAction);
